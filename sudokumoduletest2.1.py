...
def to_Box (set):
    ...
    for set_Line in set:  ## this iterates for each line of the original List
        temp_Box_1.append(set_Line[0:2])
        temp_Box_2.append(set_Line[3:5])
        temp_Box_3.append(set_Line[6:8])
    return[temp_Box1,temp_Box_2,temp_Box_3]
...
for x in to_Box (first_Set)):
    grid_Out.append(x)
for x in to_Box (_Set)):
    grid_Out.append(x)
for x in to_Box (first_Set)):
    grid_Out.append(x)

grid_print (grod_Out, 9)
